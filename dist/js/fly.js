...
FP.createFly = function (scene, world) {
  ...
  function supportY(x, z) {
    if (fly && (fly.onPillow || fly.state === 'seekRest') && world.pillowSurfaceHeight) {
      const pillowY = world.pillowSurfaceHeight(x, z);
      if (pillowY !== null) return Math.max(groundY(x, z), pillowY);
    }
    return groundY(x, z);
  }
  ...
  function stanceWorld(leg, lead, out) {
    ...
    out.y = supportY(out.x, out.z) + 0.02;
    if (fly.perchY !== null && !fly.onPillow) out.y = fly.perchY;
    return out;
  }
  ...
  fly.endCarry = function (pos) {
    ...
    if (overPillow) {
      const pillowY = world.pillowSurfaceHeight(root.position.x, root.position.z);
      root.position.y = pillowY === null ? world.pillow.worldTop : pillowY;
  ...
  function walkToward(dt, dest, speed) {
    ...
    root.position.y = fly.perchY !== null && !fly.onPillow ? fly.perchY : supportY(root.position.x, root.position.z);
  ...
  fly.update = function (dt, t, io) {
    ...
    switch (S) {
      ...
      case 'seekWater': {
        ...
        if (tmpA.lengthSq() < 1e-4) tmpA.set(1, 0, 0);
        tmpA.normalize();
        tmpD.copy(tmpA); // walkToward uses tmpA internally; preserve the radial approach vector
        tmpB.set(world.dish.x, 0, world.dish.z).addScaledVector(tmpA, world.dish.r + 0.35);
        moving = !walkToward(dt, tmpB, 1.4);
        if (!moving) {
          // hop up to the rim
          tmpC.set(world.dish.x, 0, world.dish.z).addScaledVector(tmpD, world.dish.perchR);
          root.position.set(tmpC.x, world.dish.rimY, tmpC.z);
      ...
      case 'drink': {
        ...
        if (sipTimer <= 0) {
          ...
          const sipReach = Math.max(0.7, world.dish.perchR - world.dish.waterR + 0.1);
          world.spawnRipple(tmpA.x + tmpB.x * sipReach, tmpA.z + tmpB.z * sipReach);
          fly.events.push('sip');
        }
      ...
      case 'seekRest': {
        ...
        if (d < world.pillow.r * 0.55) {
          fly.onPillow = true;
          fly.perchY = world.pillowSurfaceHeight(root.position.x, root.position.z) || world.pillow.worldTop;
          root.position.y = fly.perchY;
          for (const leg of LEGS) stanceWorld(leg, false, leg.foot);
          setState('nap');
        }
    ...
    if (fly.onPillow && world.pillowSurfaceHeight) {
      const pillowY = world.pillowSurfaceHeight(root.position.x, root.position.z);
      if (pillowY !== null) { root.position.y = pillowY; fly.perchY = pillowY; }
    }

    // ---- gait ----
    root.updateMatrixWorld(true);
    velRoot.copy(worldVel).applyQuaternion(qC.copy(root.quaternion).invert()).divideScalar(SCALE);
    if (!flying && !carried && !napping) {
    ...
    // ---- body pose ----
    let targetH = napping ? 0.12 : flying ? 0.4 : BODY_H;
    ...
    {
      const gy = fly.onPillow ? supportY(root.position.x, root.position.z) : world.groundHeight(root.position.x, root.position.z);
