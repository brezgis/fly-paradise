...
(function () {
  ...
  function initAudio() {
    ...
    saw.type = 'sawtooth'; saw.frequency.value = 205;
    const sub = audio.createOscillator();
    sub.type = 'sine'; sub.frequency.value = 102;
    const lp = audio.createBiquadFilter();
    lp.type = 'lowpass'; lp.frequency.value = 750; lp.Q.value = 1.2;
    buzzGain = audio.createGain(); buzzGain.gain.value = 0;
    const subGain = audio.createGain(); subGain.gain.value = 0.4;
  ...
  function frame() {
    ...
    brain.update(dt, elapsed, fly.activity, headPos);
    keyOrbit(dt);
    controls.update();
    ...
    for (const ev of fly.events) {
      if (ev === 'takeoff' && buzzGain && soundOn) buzzGain.gain.setTargetAtTime(0.05, audio.currentTime, 0.06);
      if (ev === 'land' && buzzGain) buzzGain.gain.setTargetAtTime(0, audio.currentTime, 0.12);
      if (ev === 'heart') blip(680, 990, 0.14, 0.05);
      if (ev === 'sip') blip(300, 210, 0.1, 0.03);
    }
    fly.events.length = 0;
    if (buzzGain && soundOn) {
      const flying = fly.state === 'fly' || fly.state === 'land';
      const target = flying ? 0.05 : 0;
      buzzGain.gain.setTargetAtTime(target, audio.currentTime, 0.1);
    }
